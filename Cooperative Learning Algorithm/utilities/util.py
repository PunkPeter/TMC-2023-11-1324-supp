            ...
            if exploration:
                if args.action_enforcebound:
                    ...
                    if info.get('clip', False):
                        actions = act_mean + \
                                  th.clamp(x_t, min=-args.clip_c, max=args.clip_c)
                    else:
        ...
        if status == 'train':
            if exploration:
                if args.epsilon_softmax:
                    ...
                    p_a = (1 - eps) * th.softmax(logits, dim=-1) + \
                          eps / logits.size(-1)
...
def translate_action(args, action, env):
    if args.continuous:
        actions = action.detach().squeeze().T
        lower_bound = env.action_lower_bound
        higher_bound = env.action_higher_bound
        a = ((higher_bound[0] - lower_bound[0]) * (actions[0]+0.4) + lower_bound[0]).round()

        channel = ((higher_bound[1] - lower_bound[1]) * actions[1] + lower_bound[1]).round()
        resolu = (higher_bound[2] - lower_bound[2]) * actions[2] + lower_bound[2]
        res = (higher_bound[3] - lower_bound[3]) * actions[3] + lower_bound[3]
        com = (higher_bound[4] - lower_bound[4]) * actions[4] + lower_bound[4]
        power = (higher_bound[5] - lower_bound[5]) * actions[5] + lower_bound[5]
        cp_actions = th.stack([a, channel, resolu, res, com, power]).T

        cp_actions = cp_actions.cpu().numpy()
        return actions.T, cp_actions
    else:
        actual = th.argmax(action, -1).detach().squeeze().cpu().numpy()
...
def prep_obs(state=[]):
    ...
    # for single transition -> batch_size=2
