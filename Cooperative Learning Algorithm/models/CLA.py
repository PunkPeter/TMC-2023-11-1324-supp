...
class CLA(Model):
    ...
    def get_actions(self, state, status, exploration, actions_avail, target=False, last_hid=None):
        ...
        if self.args.continuous:
            means, log_stds, hiddens = self.policy(state, last_hid=last_hid)
    ...
    def get_loss(self, batch):
        ...
        _, actions_pol, log_prob_a, action_out, _ = self.get_actions(state, status='train', exploration=True,
        ...
        compose = self.value(state, actions.detach())
        ...
        returns = rewards + self.args.gamma * (
                1 - done) * (next_values.detach() - self.args.soft * log_prob_a / self.args.reward_scale)

        deltas = returns - values
        all_values_pol = th.tensor([th.sum(i) for i in values_pol])
        counter_baseline = th.zeros(batch_size, self.n_)
        for i in range(batch_size):
            for j in range(self.n_):
                counter_baseline[i][j] = (all_values_pol[i] - values_pol[i][j]) / (self.n_ - 1)

        advantages = values_pol.to(self.device) - counter_baseline.to(self.device)  # Counterfactual baseline
        if self.args.normalize_advantages:
            advantages = self.batchnorm(advantages)
        if self.args.soft:
            global COUNTER
            COUNTER += 1
            policy_loss = ((0.99 ** (COUNTER//600)) * log_prob_a.detach() / self.args.reward_scale - advantages.detach()) * log_prob_a
        else:
            policy_loss = - advantages.detach() * log_prob_a

        policy_loss = policy_loss.mean()
        value_loss = deltas.pow(2).mean(dim=0)
        value_loss = th.sum(value_loss)
