...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.Transition = namedtuple('Transition', ('state', 'action', 'log_prob_a', 'value', 'next_value',
                                                    'reward', 'cost', 'next_state', 'done', 'last_step', 'action_avail',
                                                    'last_hid', 'hid'))
    ...
    def train_process(self, stat, episode_i, trainer):
        ...
        # state, global_state = trainer.env.manual_reset(199, 23, 4)

        # init hidden states
        # last_hid = self.policy_dicts[0].init_hidden()
        last_hid = th.zeros(1, self.n_, self.args.hid_size)
        for t in range(self.args.max_steps):
            # current state, action, value
            state_ = prep_obs(state).to(self.device).contiguous().view(
                1, self.n_, self.obs_dim)
            with th.no_grad():
                action, action_pol, log_prob_a, _, hid = self.get_actions(state_, status='train', exploration=True,
                                                                          actions_avail=th.tensor(
                                                                              trainer.env.get_avail_actions()),
                                                                          target=False, last_hid=last_hid)
            ...
            # reward
            state_, reward, done, cost = trainer.env.step(actual)
            if trainer.env.independ_reward:
                reward_repeat = reward
            else:
                reward_repeat = [reward] * trainer.env.get_num_of_agents()

            # get next state, action, value
            next_state = state_  # trainer.env.get_obs()
            next_state_ = prep_obs(next_state).to(
                self.device).contiguous().view(1, self.n_, self.obs_dim)
            with th.no_grad():
                _, next_action_pol, _, _, _ = self.get_actions(next_state_, status='train', exploration=True,
                                                               actions_avail=th.tensor(
                                                                   trainer.env.get_avail_actions()), target=False,
                                                               last_hid=hid)
            ...
            print("Episode ", episode_i, " Step ", t, " Rewards: ", reward_repeat)
            if not self.args.episodic:
                self.transition_update(
                    trainer, trans, stat, 1)  # , info["destroy"])
            else:
                episode.append(trans)
            stat_train['mean_train_reward'] += np.mean(reward)
            # if not self.args.safe_trans or info["totally_controllable_ratio"] == 2.:
            trainer.steps += 1
            if done_:
                break
            # set the next state
        ...
        with th.no_grad():
            for _ in range(num_eval_episodes):
                ...
                for t in range(self.args.max_eval_steps):
                    state_ = prep_obs(state).to(self.device).contiguous().view(
                        1, self.n_, self.obs_dim)
                    action, _, _, _, hid = self.get_actions(state_, status='test', exploration=False,
                                                            actions_avail=th.tensor(
                                                                trainer.env.get_avail_actions()), target=False,
                                                            last_hid=last_hid)
                    ...
                    #         label_v = state[-4*len(trainer.env.base_powergrid.bus):-len(trainer.env.base_powergrid.bus)]
                    #         state = th.tensor(state).to(th.float32).to(self.device)[None,:]
                    #         q = th.tensor(q).to(th.float32).to(self.device)[None,:]
                    #         pred_v = constraint_model(th.cat((state,q),dim=2)).detach().squeeze().cpu().numpy()
    ...
    def unpack_data(self, batch):
        ...
        cost = None if sum([cost_ is None for cost_ in batch.cost]) else th.tensor(batch.cost, dtype=th.float).to(self.device)
        ...
        return (
            state, action, log_prob_a, value, next_value, reward, cost, next_state, done, last_step, action_avail,
            last_hid,
            hid)
