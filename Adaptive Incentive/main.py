...
from utils.sc_utils import wei_to_ether
...
if __name__ == "__main__":
    ...
    w3 = web3.Web3(web3.HTTPProvider('http://127.0.0.1:7549', request_kwargs={'timeout': 60 * 10}))

    contract_source_path = './contracts/Operations.sol'
    compiled_sol = compile_source_file(contract_source_path)

    contract_id, contract_interface = compiled_sol.popitem()
    address = deploy_contract(w3, contract_interface)
    print("The name of the compiled contract:{0}succeed and deployed at : {1}\n".format(contract_id, address))
    ...
    print("Initial contract balance:", wei_to_ether(w3.eth.get_balance(address)))
    print("Initial account balance:", wei_to_ether(w3.eth.get_balance(w3.eth.accounts[1])))
    ...
    print("Contract balance after receiving the deposit:",
          wei_to_ether(w3.eth.get_balance(w3.eth.accounts[1])))
    print("Account balance after submitting the deposit:",
          wei_to_ether(w3.eth.get_balance(w3.eth.accounts[1])))
    print("The initial deposit:", wei_to_ether(deposits))

    print("\nInitialization of object detection on COCO dataset...")
    opt = parse_opt()

    conf["compute_power"] = main(opt)
    ...
    print("The balance of all accounts after the contract is calculated:")
    print("Contract balance:", wei_to_ether(w3.eth.get_balance(address)))
    new_balance = []
    for i in range(conf['client_total_num']):
        if deposits[i] > reward[i]:
            print("Account", i, "is charged", wei_to_ether(deposits[i] - reward[i]),
                  "ether, with balance:", wei_to_ether(w3.eth.get_balance(w3.eth.accounts[i])))
        else:
            print("Account", i, "get the deposit",
                  wei_to_ether(reward[i] - deposits[i]), "ether, with balance:",
                  wei_to_ether(w3.eth.get_balance(w3.eth.accounts[i])))
